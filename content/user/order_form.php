...
<main>
    <div class="container my-5 p-3 mnb-nm rounded">
        ...
        <form method="POST" action="/content/script/s_order.php">
            <div class="d-flex column justify-content-around">
                ...
                <div class="col-5">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" min=1 class="form-control" id="quantite" name="quantite" value=0 required>
                    </div>
                    <div class="mt-4">
                        <span id="recap"></span>
                    </div>
            ...
            <div class="d-flex justify-content-center">
                <button type="submit" name="submit" class="btn btn-light btn-sm text-black mb-1 mt-3">Passer la commande</button>
            </div>
        </form>
    </div>
</main>
...
<script>
    quantite.addEventListener("input", function() {
        total(parseInt(document.getElementById('quantite').value))
    });

    function total(input_quantite) {
        document.getElementById('recap').innerHTML = "";
        if (!isNaN(input_quantite)) {
            let e = document.getElementById('plat');
            let price = 0;
            let regex = /(\d+\.\d+)/;
            let matches = regex.exec(e[e.selectedIndex].innerHTML);
            if (matches) {
                price = matches[0];
            }
